"""EXP-2026-08-29-AMIHUD-001: Cross-sectional Amihud illiquidity premium.

Hypothesis (preregistered, genuinely new -- not previously tested in this
repo's docs/experiment_registry.md):
  Illiquid assets should command a return premium over liquid assets because
  investors demand compensation for the higher price-impact cost of trading
  them (classic Amihud 2002 illiquidity-premium mechanism, applied
  cross-sectionally to a real Binance spot daily universe). Ranking assets by
  a rolling Amihud illiquidity ratio (mean |daily return| / dollar volume) and
  going long the most-illiquid tercile / short the most-liquid tercile
  (dollar-neutral) should earn a premium net of realistic round-trip costs.

Design (frozen before any result was inspected):
  - Universe: same 30-coin real Binance spot USDT daily universe already
    cached in data/raw/*_1d.csv.gz (10 core + 20 fetched for the residual-
    momentum study), 2020-01-01 through repo cutoff.
  - Amihud illiquidity ratio per asset per day:
        illiq_t = mean_{i in window}( |r_i| / dollar_volume_i )
    where r_i = daily simple return, dollar_volume_i = close_i * volume_i
    (real Binance base-asset volume; no quote_volume column cached, so
    dollar volume is approximated as close*volume, consistent with standard
    Amihud implementations when true quote volume is unavailable).
  - Rolling window: 14 completed days (min 10 obs), computed through day t-1
    (shift(1) applied) so the rebalance-day score never uses same-day data.
  - Rebalance: weekly (every 7 days), long top tercile (most illiquid) /
    short bottom tercile (most liquid) by score, equal-weighted within each
    leg, dollar-neutral (long leg and short leg each sized at 50% of gross
    capital, net market exposure ~0). Enter at next day's open, hold to next
    rebalance's entry.
  - Costs: repo-standard 30bps round-trip (10bps fee + 5bps slippage per
    side, matching FEE_RATE/SLIPPAGE_RATE), charged on turnover fraction of
  ...
  - Control: seeded random-ranking L/S with identical leg sizes/turnover/cost
    structure, to isolate whether the illiquidity ranking itself adds value.
  - Benchmarks: cash, BTC/ETH/SOL/XRP buy-and-hold, equal-weight-30
    buy-and-hold.

Fastest rejection criterion (preregistered): illiquidity-premium L/S must
beat the random-ranking control AND cash after costs, or it is rejected.
...
def build_illiquidity_score(closes: pd.DataFrame, volumes: pd.DataFrame) -> pd.DataFrame:
    ...
    # avoid division by zero / negative dollar volume
...
def run_ls_backtest(score: pd.DataFrame, opens: pd.DataFrame, label: str) -> dict:
    ...
    prev_weights: dict[str, float] = {}  # signed weights: + long leg, - short leg
    equity_curve = []
    trade_log = []
    ...
    for i, reb_date in enumerate(reb_dates):
        ...
        long_names = ranked.head(leg_n).index.tolist()   # most illiquid
        short_names = ranked.tail(leg_n).index.tolist()  # most liquid
...
def main() -> None:
    ...
    result_strategy = run_ls_backtest(score, opens, "amihud_illiquidity_ls")
    ...
    out_dir = ROOT / "results" / "amihud_illiquidity_cross_sectional" / "runs"
    ...
    summary_rows = [
        {"strategy": "Amihud illiquidity L/S (long illiquid, short liquid)", **metrics_strategy},
