"""EXP-2026-08-30-AMIHUD-SMALLCAP-001: Cross-sectional Amihud illiquidity premium
on a genuinely thin (~rank 100-400 market cap) crypto universe.

Hypothesis (preregistered, follow-up to already-REJECTED EXP-2026-08-29-AMIHUD-001):
  The top-30-market-cap Amihud illiquidity study failed to beat cash. This skill's
  own prior guidance (and the reference literature) holds that illiquidity premia
  should be STRONGER among genuinely thin names, not top-30 majors which are all
  fairly liquid to begin with. This is a NEW, distinct universe segment test of the
  identical mechanism -- not a retune of the rejected top-30 configuration.

Universe construction (frozen before any result was inspected):
  - Live CoinGecko market-cap ranking (`coins/markets?order=market_cap_desc`),
    filtered to rank 100-400.
  - Cross-referenced against live Binance spot USDT-quoted TRADING pairs
    (`api.binance.com/api/v3/exchangeInfo`) to ensure genuine spot tradability.
  - Stablecoins / wrapped / liquid-staking / RWA tickers explicitly excluded
    (USDC, USDT, DAI, WBTC, stETH, etc.) -- not tradable factor exposure.
  - Final universe: up to ~55 real coins fetched via real Binance spot daily
    OHLCV klines (see fetch_smallcap.py), 2018-01-01 through repo cutoff
    (2026-07-28), each requiring >= 300 rows of real history to be included.

Design (identical mechanism/parameters to the rejected top-30 study, EXP-2026-08-29-AMIHUD-001,
except universe -- so any different result is attributable to universe thinness, not
methodology tuning):
  - Illiquidity ratio: mean(|daily return| / dollar_volume) over rolling 14-day
    window (min 10 obs), shifted 1 day (no lookahead).
  - Rebalance every 7 days, long top tercile (most illiquid) / short bottom
    tercile (most liquid), equal-weighted within each leg, 50%/50% dollar-neutral
    gross split. Enter at next day's open, hold to next rebalance's entry.
  - Costs: repo-standard 30bps round-trip, charged on turnover fraction of gross
    notional that changes membership.
  - Control: seeded random-ranking L/S, identical leg sizes/turnover/cost structure.
  - Benchmarks: cash, BTC/ETH/SOL/XRP buy-and-hold, equal-weight-universe buy-and-hold.

Fastest rejection criterion (preregistered, same as prior study): illiquidity-premium
L/S must beat the random-ranking control AND cash after costs, and no single trade
may exceed 20% of total strategy net PnL (concentration cap), or it is REJECTED.
"""
...
def discover_universe() -> list[str]:
    """Any *_1d.csv.gz file freshly fetched by fetch_smallcap.py, real Binance data only."""
    candidates_path = Path("/tmp/fetch_smallcap.log")
    coins = []
    if candidates_path.exists():
        for line in candidates_path.read_text().splitlines():
            parts = line.split(":")
            if len(parts) >= 2 and "rows" in parts[1] and "FAILED" not in line and "too short" not in line:
                coin = parts[0].strip()
                if (PATHS.raw / f"{coin}_1d.csv.gz").exists():
                    coins.append(coin)
...
def main() -> None:
    ...
    print(f"Discovered universe ({len(universe)} coins): {universe}")
    if len(universe) < 15:
        print("ERROR: universe too small for tercile ranking (<15 coins). Aborting.")
        sys.exit(1)
    ...
    result_strategy = run_ls_backtest(score, opens, "amihud_illiquidity_ls_smallcap")
    ...
    # Core benchmarks: BTC/ETH/SOL/XRP from the repo's existing core universe (not part of this
    # small/mid-cap factor universe, loaded separately).
    core = load_field(["BTC", "ETH", "SOL", "XRP"], "close")
    btc_bh = buy_and_hold(core, "BTC")
    eth_bh = buy_and_hold(core, "ETH")
    sol_bh = buy_and_hold(core, "SOL")
    xrp_bh = buy_and_hold(core, "XRP")
    ...
    summary_rows = [
        {"strategy": "Amihud illiquidity L/S small/mid-cap (long illiquid, short liquid)", **metrics_strategy},
        {"strategy": "Random-ranking L/S control (same leg sizes/turnover)", **metrics_random},
        {"strategy": f"Equal-weight {len(universe)}-asset small/mid-cap buy-and-hold", **ew_bh,
         "n_trades": 1, "win_rate": float("nan"), "top_trade_pct_of_pnl": float("nan")},
