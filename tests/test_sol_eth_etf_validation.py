...
from crypto_regime_backtest.sol_eth_etf_validation import StrategySpec, build_sol_etf_signals


def test_build_sol_etf_signals_uses_completed_24h_move_and_next_open_entry():
    ...
    signals = build_sol_etf_signals(frame, spec)
...
def test_build_sol_etf_signals_respects_level_gate():
    timestamps = pd.date_range("2025-01-01", periods=30, freq="h", tz="UTC")
    frame = pd.DataFrame(
        {
            "timestamp": timestamps,
