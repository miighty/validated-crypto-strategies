"""EXP-2026-09-06-BAACREDITRISKOFF-001: BAA credit-spread stress as crypto risk-off overlay.

Preregistered rule: use real FRED Moody's Seasoned BAA Corporate Bond Yield
minus real FRED 10-Year Treasury Constant Maturity (DBAA - DGS10) as an external
credit-stress de-risking overlay. If the spread z-score is >= +2.0 versus a
...
BASE_SEED = 202609062
FRED_SERIES = "DBAA,DGS10"
FRED_URL = f"https://fred.stlouisfed.org/graph/fredgraph.csv?id={FRED_SERIES}"
CACHE = ROOT / "data" / "macro_credit" / "baa_treasury_spread_1d.csv.gz"
REPORT_PATH = ROOT / "docs" / "BAA_CREDIT_SPREAD_RISKOFF_VALIDATION.md"
RESULTS_DIR = ROOT / "results" / "baa_credit_spread_riskoff"


def fetch_or_load_credit_spread() -> pd.DataFrame:
    ...
    raw.columns = ["date", "baa_yield", "teny_yield"]
    raw = raw[(raw["baa_yield"] != ".") & (raw["teny_yield"] != ".")].copy()
    raw["date"] = pd.to_datetime(raw["date"], utc=True)
    raw["baa_yield"] = raw["baa_yield"].astype(float)
    raw["teny_yield"] = raw["teny_yield"].astype(float)
    raw["credit_spread"] = raw["baa_yield"] - raw["teny_yield"]
...
def build_credit_state(spread: pd.DataFrame) -> pd.DataFrame:
    df = spread.set_index("date").sort_index()
    df = df[df.index < END_EXCLUSIVE].copy()
    df = df.dropna(subset=["credit_spread"])
    prior = df["credit_spread"].shift(1)
    ...
    df["z"] = (df["credit_spread"] - df["mean"]) / df["std"]
    risk_off = False
    states: list[bool] = []
    for z in df["z"]:
        if np.isfinite(z):
...
def write_report(results: list[dict], state: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        f"- **Real data:** FRED DBAA and DGS10 cached at `{CACHE.relative_to(ROOT)}` ({state.index.min().date()} to {state.index.max().date()}); cached Binance spot daily OHLCV; no synthetic/proxy inputs.",
        "", "## Results", "",
...
def main() -> None:
    spread = fetch_or_load_credit_spread()
    state = build_credit_state(spread)
    results = [evaluate_asset(asset, state) for asset in ASSETS]
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    out = pd.DataFrame(results)
    out.to_csv(RESULTS_DIR / "summary.csv", index=False)
    state.to_csv(RESULTS_DIR / "baa_credit_spread_state.csv")
