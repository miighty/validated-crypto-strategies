"""EXP-2026-09-08-UMCSENT-001: consumer-sentiment risk-on regime filter.

Preregistered rule: use real FRED University of Michigan Consumer Sentiment (UMCSENT)
monthly data as an external household-risk-appetite filter. Hold BTC/ETH/SOL/XRP long
only when the latest actionable monthly UMCSENT print is above its prior-only 12-month
SMA and its prior-only 3-month change is positive; otherwise cash. No synthetic/proxy data.
"""
...
CACHE = ROOT / "data" / "macro_consumer_sentiment" / "umcsent_1mo.csv.gz"
REPORT_PATH = ROOT / "docs" / "CONSUMER_SENTIMENT_REGIME_VALIDATION.md"
RESULTS_DIR = ROOT / "results" / "consumer_sentiment_regime"


def fetch_or_load_umcsent() -> pd.DataFrame:
    CACHE.parent.mkdir(parents=True, exist_ok=True)
    if CACHE.exists():
        df = pd.read_csv(CACHE, parse_dates=["date"])
        df["date"] = pd.to_datetime(df["date"], utc=True)
        df["umcsent"] = pd.to_numeric(df["umcsent"], errors="coerce")
        df = df.dropna(subset=["umcsent"]).sort_values("date").drop_duplicates("date").reset_index(drop=True)
        if df["date"].max() >= END_EXCLUSIVE - pd.Timedelta(days=90):
            return df
    payload = None
    ...
    if payload is None:
        raise RuntimeError(f"Real FRED UMCSENT fetch failed: {last_error}")
    raw = pd.read_csv(StringIO(payload))
    raw.columns = ["date", "umcsent"]
    raw["date"] = pd.to_datetime(raw["date"], utc=True)
    raw["umcsent"] = pd.to_numeric(raw["umcsent"], errors="coerce")
    raw = raw.dropna(subset=["umcsent"]).sort_values("date").drop_duplicates("date").reset_index(drop=True)
...
def build_state(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    prior = df["umcsent"].shift(1)
    df["sma12_prior"] = prior.rolling(12, min_periods=12).mean()
    df["change3_prior"] = prior - df["umcsent"].shift(4)
    df["risk_on_signal"] = ((df["umcsent"] > df["sma12_prior"]) & (df["change3_prior"] > 0)).fillna(False).astype(bool)
    # Monthly macro release/action lag: act only after one completed print has passed.
...
def write_report(results: list[dict], state: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        f"- **Real data:** FRED UMCSENT cached at `{CACHE.relative_to(ROOT)}` ({state.index.min().date()} to {state.index.max().date()}); cached Binance spot daily OHLCV; no synthetic/proxy inputs.",
        "",
        "## Results",
        "",
...
def main() -> None:
    ...
    state.to_csv(RESULTS_DIR / "umcsent_state.csv")
