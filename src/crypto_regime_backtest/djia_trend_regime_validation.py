"""EXP-2026-09-08-DJIATREND-001: Dow Jones Industrial Average trend risk-on regime filter.

Preregistered rule: use real FRED DJIA daily closes only as an external blue-chip
risk-appetite filter. Hold BTC/ETH/SOL/XRP long only when the Dow Jones Industrial Average close is above
its prior-only 200-trading-day SMA; otherwise cash. The crypto action is delayed one
daily bar after the DJIA close signal. No synthetic/proxy data.
"""
...
BASE_SEED = 202609082
DJIA_URL = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=DJIA"
DJIA_CACHE = ROOT / "data" / "macro_djia_trend" / "djia_1d.csv.gz"
REPORT_PATH = ROOT / "docs" / "DJIA_TREND_REGIME_VALIDATION.md"
RESULTS_DIR = ROOT / "results" / "djia_trend_regime"


def fetch_or_load_djia() -> pd.DataFrame:
    DJIA_CACHE.parent.mkdir(parents=True, exist_ok=True)
    if DJIA_CACHE.exists():
        df = pd.read_csv(DJIA_CACHE, parse_dates=["date"])
        df["date"] = pd.to_datetime(df["date"], utc=True)
        df["djia"] = pd.to_numeric(df["djia"], errors="coerce")
        df = df.dropna(subset=["djia"]).sort_values("date").drop_duplicates("date").reset_index(drop=True)
        df.to_csv(DJIA_CACHE, index=False, compression="gzip")
    ...
    for attempt in range(5):
        try:
            req = urllib.request.Request(DJIA_URL, headers={"User-Agent": "validated-crypto-strategies/0.1"})
    ...
    if payload is None:
        raise RuntimeError(f"Real FRED DJIA fetch failed: {last_error}")
    raw = pd.read_csv(StringIO(payload))
    raw.columns = ["date", "djia"]
    raw["date"] = pd.to_datetime(raw["date"], utc=True)
    raw["djia"] = pd.to_numeric(raw["djia"], errors="coerce")
    raw = raw.dropna(subset=["djia"])
    raw = raw.sort_values("date").drop_duplicates("date").reset_index(drop=True)
    raw.to_csv(DJIA_CACHE, index=False, compression="gzip")
    return raw
...
def build_djia_state(djia: pd.DataFrame) -> pd.DataFrame:
    df = djia.set_index("date").sort_index()
    df = df[df.index < END_EXCLUSIVE].copy()
    prior = df["djia"].shift(1)
    df["sma200_prior"] = prior.rolling(ROLLING_DAYS, min_periods=ROLLING_DAYS).mean()
    df["risk_on_signal_close"] = (df["djia"] > df["sma200_prior"]).fillna(False).astype(bool)
...
def write_report(results: list[dict], state: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        f"- **Real data:** FRED DJIA cached at `{DJIA_CACHE.relative_to(ROOT)}` ({state.index.min().date()} to {state.index.max().date()}); cached Binance spot daily OHLCV; no synthetic/proxy inputs.",
        "",
        "## Results",
        "",
