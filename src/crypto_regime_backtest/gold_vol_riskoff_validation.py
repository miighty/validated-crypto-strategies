"""EXP-2026-09-07-GOLDVOLRISKOFF-001: gold-implied-volatility risk-off overlay.

Preregistered rule: use real FRED CBOE Gold ETF Volatility Index (GVZCLS) as
an external cross-asset stress signal. If GVZ z-score is >= +2.0 versus a
prior-only 252-trading-day baseline, exit BTC/ETH/SOL/XRP at the next daily open
and stay in cash until z < +1.0; then re-enter at the next daily open. No
...
CACHE = ROOT / "data" / "macro_gold_vol" / "gvzcls_1d.csv.gz"
REPORT_PATH = ROOT / "docs" / "GOLD_VOL_RISKOFF_VALIDATION.md"
RESULTS_DIR = ROOT / "results" / "gold_vol_riskoff"


def fetch_or_load_gvz() -> pd.DataFrame:
    CACHE.parent.mkdir(parents=True, exist_ok=True)
    if CACHE.exists():
        df = pd.read_csv(CACHE, parse_dates=["date"])
        df["date"] = pd.to_datetime(df["date"], utc=True)
        if df["date"].min() <= pd.Timestamp("2008-06-03T00:00:00Z") and df["date"].max() >= END_EXCLUSIVE - pd.Timedelta(days=14):
            return df
    payload = None
    ...
    raw.columns = ["date", "gold_vol"]
    raw = raw[raw["gold_vol"] != "."].copy()
    raw["date"] = pd.to_datetime(raw["date"], utc=True)
    raw["gold_vol"] = pd.to_numeric(raw["gold_vol"], errors="coerce")
    raw = raw.dropna(subset=["gold_vol"]).sort_values("date").drop_duplicates("date").reset_index(drop=True)
...
def build_gvz_state(gvz: pd.DataFrame) -> pd.DataFrame:
    df = gvz.set_index("date").sort_index()
    df = df[df.index < END_EXCLUSIVE].dropna(subset=["gold_vol"]).copy()
    prior = df["gold_vol"].shift(1)
    ...
    df["z"] = (df["gold_vol"] - df["mean"]) / df["std"]
    risk_off = False
    states: list[bool] = []
    for z in df["z"]:
        if np.isfinite(z):
...
def write_report(results: list[dict], state: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        f"- **Real data:** FRED GVZCLS cached at `{CACHE.relative_to(ROOT)}` ({state.index.min().date()} to {state.index.max().date()}); cached Binance spot daily OHLCV; no synthetic/proxy inputs.",
        "",
        "## Results",
        "",
