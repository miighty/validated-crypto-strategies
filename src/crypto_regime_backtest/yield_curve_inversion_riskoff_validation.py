"""EXP-2026-09-06-YIELDCURVE-001: Treasury yield-curve inversion as crypto risk-off overlay.

Preregistered rule: use real FRED 10-Year Treasury Constant Maturity minus real FRED
2-Year Treasury Constant Maturity (DGS10 - DGS2) as an external macro
cycle-stress de-risking overlay. If the term-spread z-score is <= -2.0 versus a
prior-only 252-trading-day baseline, exit BTC/ETH/SOL/XRP at the next daily open;
stay cash until z > -1.0, then re-enter at the next daily open. No synthetic or
proxy market inputs.
"""
...
CACHE = ROOT / "data" / "macro_credit" / "treasury_10y2y_spread_1d.csv.gz"
REPORT_PATH = ROOT / "docs" / "YIELD_CURVE_INVERSION_RISKOFF_VALIDATION.md"
RESULTS_DIR = ROOT / "results" / "yield_curve_inversion_riskoff"


def fetch_or_load_yield_curve() -> pd.DataFrame:
    CACHE.parent.mkdir(parents=True, exist_ok=True)
    if CACHE.exists():
        df = pd.read_csv(CACHE, parse_dates=["date"])
        df["date"] = pd.to_datetime(df["date"], utc=True)
        if df["date"].min() <= pd.Timestamp("1997-01-01T00:00:00Z") and df["date"].max() >= END_EXCLUSIVE - pd.Timedelta(days=90):
            return df
    payload = None
    ...
    raw.columns = ["date", "teny_yield", "twoy_yield"]
    raw = raw[(raw["teny_yield"] != ".") & (raw["twoy_yield"] != ".")].copy()
    raw["date"] = pd.to_datetime(raw["date"], utc=True)
    raw["teny_yield"] = raw["teny_yield"].astype(float)
    raw["twoy_yield"] = raw["twoy_yield"].astype(float)
    raw["term_spread"] = raw["teny_yield"] - raw["twoy_yield"]
...
def build_yield_curve_state(spread: pd.DataFrame) -> pd.DataFrame:
    df = spread.set_index("date").sort_index()
    df = df[df.index < END_EXCLUSIVE].copy()
    df = df.dropna(subset=["term_spread"])
...
def simulate_regime(spot: pd.DataFrame, long_allowed: pd.Series, one_way_cost: float) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for ts, row in spot.iterrows():
        ...
        if in_pos and not allowed:
            ...
            trades.append({"entry_time": entry_time, "exit_time": ts, "entry_price": entry_price, "exit_price": px,
                           "entry_capital": entry_capital, "exit_capital": exit_capital,
                           "pnl": exit_capital - float(entry_capital), "net_return": exit_capital / float(entry_capital) - 1.0})
...
def evaluate_asset(asset: str, state: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "risk_off_days": int(risk_off.sum()), "risk_off_fraction": float(risk_off.mean()), "trades": int(len(trades)),
        "final": final, "buy_hold": buy_hold_final(spot), "dca": daily_dca_final(spot),
        "momentum": float(mom_eq["equity"].iloc[-1]), "random": float(rand_eq["equity"].iloc[-1]),
        "double_cost": float(eq_double["equity"].iloc[-1]), "delay_2d": float(eq_delay["equity"].iloc[-1]),
        "best_excluded": best_excluded_final, "top_block_pct": top_block_pct,
        "sharpe": sharpe_from_equity(eq["equity"]), "max_drawdown": max_drawdown(eq["equity"]),
...
def write_report(results: list[dict], state: pd.DataFrame) -> None:
    ...
    for r in results:
        lines.append(
            f"| {r['asset']} | {r['risk_off_days']} ({r['risk_off_fraction']:.1%}) | {r['trades']} | ${r['final']:,.2f} | ${r['buy_hold']:,.2f} | ${r['dca']:,.2f} | ${r['momentum']:,.2f} | ${r['random']:,.2f} | ${r['double_cost']:,.2f} | ${r['delay_2d']:,.2f} | ${r['test_final']:,.2f} vs ${r['test_buy_hold']:,.2f} ({r['test_trades']} trades) | {r['sharpe']:.2f} | {r['max_drawdown']:.1%} | {r['top_block_pct']:.1%} | Inconclusive |"
        )
    lines.extend([
        "", "## Decisive checks", "",
        f"- Benchmark gate passed on headline capital: {beat_bh}/4 beat buy-and-hold, {beat_dca}/4 beat DCA, {beat_mom}/4 beat the BTC-momentum regime control.",
        f"- Concentration gate failed: {pass_conc}/4 assets cleared the 20% top-PnL-block cap; observed top-block shares are 52%-71%.",
        f"- Test partition evidence is too sparse: {beat_test}/4 assets beat their own 2024+ buy-and-hold benchmark, but each asset has only one 2024+ exposure block.",
