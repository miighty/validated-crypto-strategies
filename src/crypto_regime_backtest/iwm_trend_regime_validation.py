"""EXP-2026-09-08-IWMTREND-001: Russell 2000 / IWM trend risk-on regime filter.

Preregistered rule: use real Yahoo Finance IWM adjusted daily closes only as an external small-cap-equity
risk appetite filter. Hold BTC/ETH/SOL/XRP long only when IWM adjusted close is above
its prior-only 200-trading-day SMA; otherwise cash. The crypto action is delayed one
daily bar after the IWM close signal. No synthetic/proxy data.
"""
...
BASE_SEED = 202609085
IWM_CACHE = ROOT / "data" / "macro_iwm_trend" / "iwm_1d.csv.gz"
REPORT_PATH = ROOT / "docs" / "IWM_TREND_REGIME_VALIDATION.md"
RESULTS_DIR = ROOT / "results" / "iwm_trend_regime"


def fetch_or_load_iwm() -> pd.DataFrame:
    IWM_CACHE.parent.mkdir(parents=True, exist_ok=True)
    if IWM_CACHE.exists():
        df = pd.read_csv(IWM_CACHE, parse_dates=["date"])
        df["date"] = pd.to_datetime(df["date"], utc=True)
        df["iwm"] = pd.to_numeric(df["iwm"], errors="coerce")
        df = df.dropna(subset=["iwm"]).sort_values("date").drop_duplicates("date").reset_index(drop=True)
    ...
    for attempt in range(5):
        try:
            period1 = int(pd.Timestamp("2000-05-26", tz="UTC").timestamp())
            period2 = int(END_EXCLUSIVE.timestamp())
            url = f"https://query1.finance.yahoo.com/v8/finance/chart/IWM?period1={period1}&period2={period2}&interval=1d&events=history&includeAdjustedClose=true"
    ...
    if payload is None:
        raise RuntimeError(f"Real Yahoo Finance IWM fetch failed: {last_error}")
    import json
    ...
    raw = pd.DataFrame({"date": pd.to_datetime(ts, unit="s", utc=True).normalize(), "iwm": quote})
    raw["iwm"] = pd.to_numeric(raw["iwm"], errors="coerce")
    raw = raw.dropna(subset=["iwm"]).sort_values("date").drop_duplicates("date").reset_index(drop=True)
    raw.to_csv(IWM_CACHE, index=False, compression="gzip")
    return raw
...
def build_iwm_state(iwm: pd.DataFrame) -> pd.DataFrame:
    df = iwm.set_index("date").sort_index()
    df = df[df.index < END_EXCLUSIVE].copy()
    prior = df["iwm"].shift(1)
    df["sma200_prior"] = prior.rolling(ROLLING_DAYS, min_periods=ROLLING_DAYS).mean()
    df["risk_on_signal_close"] = (df["iwm"] > df["sma200_prior"]).fillna(False).astype(bool)
...
def write_report(results: list[dict], state: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        f"- **Real data:** Yahoo Finance IWM adjusted close cached at `{IWM_CACHE.relative_to(ROOT)}` ({state.index.min().date()} to {state.index.max().date()}); cached Binance spot daily OHLCV; no synthetic/proxy inputs.",
        "",
        "## Results",
        "",
    ...
    lines.extend([
        ...
        "## Decisive checks",
        "",
        f"- Deployability gate failed despite B&H wins: {beat_bh}/4 beat buy-and-hold, {beat_dca}/4 beat DCA, but only {beat_mom}/4 beat the BTC-momentum regime control.",
