"""EXP-2026-09-08-SP500TREND-001: S&P 500 trend risk-on regime filter.

Preregistered rule: use real FRED SP500 daily closes only as an external broad-equity
risk appetite filter. Hold BTC/ETH/SOL/XRP long only when the S&P 500 close is above
its prior-only 200-trading-day SMA; otherwise cash. The crypto action is delayed one
daily bar after the SP500 close signal. No synthetic/proxy data.
"""
...
SP500_CACHE = ROOT / "data" / "macro_sp500_trend" / "sp500_1d.csv.gz"
REPORT_PATH = ROOT / "docs" / "SP500_TREND_REGIME_VALIDATION.md"
RESULTS_DIR = ROOT / "results" / "sp500_trend_regime"


def fetch_or_load_sp500() -> pd.DataFrame:
    SP500_CACHE.parent.mkdir(parents=True, exist_ok=True)
    if SP500_CACHE.exists():
        df = pd.read_csv(SP500_CACHE, parse_dates=["date"])
        df["date"] = pd.to_datetime(df["date"], utc=True)
        df["sp500"] = pd.to_numeric(df["sp500"], errors="coerce")
        df = df.dropna(subset=["sp500"]).sort_values("date").drop_duplicates("date").reset_index(drop=True)
        df.to_csv(SP500_CACHE, index=False, compression="gzip")
    ...
    for attempt in range(5):
        try:
            req = urllib.request.Request(SP500_URL, headers={"User-Agent": "validated-crypto-strategies/0.1"})
    ...
    if payload is None:
        raise RuntimeError(f"Real FRED SP500 fetch failed: {last_error}")
    raw = pd.read_csv(StringIO(payload))
    raw.columns = ["date", "sp500"]
    raw["date"] = pd.to_datetime(raw["date"], utc=True)
    raw["sp500"] = pd.to_numeric(raw["sp500"], errors="coerce")
    raw = raw.dropna(subset=["sp500"])
    raw = raw.sort_values("date").drop_duplicates("date").reset_index(drop=True)
    raw.to_csv(SP500_CACHE, index=False, compression="gzip")
    return raw
...
def build_sp500_state(sp500: pd.DataFrame) -> pd.DataFrame:
    df = sp500.set_index("date").sort_index()
    df = df[df.index < END_EXCLUSIVE].copy()
    prior = df["sp500"].shift(1)
    df["sma200_prior"] = prior.rolling(ROLLING_DAYS, min_periods=ROLLING_DAYS).mean()
    df["risk_on_signal_close"] = (df["sp500"] > df["sma200_prior"]).fillna(False).astype(bool)
...
def write_report(results: list[dict], state: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        f"- **Real data:** FRED SP500 cached at `{SP500_CACHE.relative_to(ROOT)}` ({state.index.min().date()} to {state.index.max().date()}); cached Binance spot daily OHLCV; no synthetic/proxy inputs.",
        "",
        "## Results",
        "",
