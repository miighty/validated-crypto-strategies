"""EXP-2026-09-07-BREAKEVEN-RISKOFF-001: 10Y breakeven-inflation growth-scare overlay.

Preregistered rule: use real FRED 10-Year Breakeven Inflation Rate (T10YIE) as
an external macro growth/liquidity stress signal. If breakeven-inflation z-score
is <= -2.0 versus a prior-only 252-trading-day baseline, exit BTC/ETH/SOL/XRP at
the next daily open and stay in cash until z > -1.0; then re-enter at the next
daily open. No synthetic/proxy inputs.
"""
...
CACHE = ROOT / "data" / "macro_breakeven_inflation" / "t10yie_1d.csv.gz"
REPORT_PATH = ROOT / "docs" / "BREAKEVEN_INFLATION_RISKOFF_VALIDATION.md"
RESULTS_DIR = ROOT / "results" / "breakeven_inflation_riskoff"


def fetch_or_load_t10yie() -> pd.DataFrame:
    ...
    raw.columns = ["date", "breakeven_inflation"]
    raw = raw[raw["breakeven_inflation"] != "."].copy()
    raw["date"] = pd.to_datetime(raw["date"], utc=True)
    raw["breakeven_inflation"] = raw["breakeven_inflation"].astype(float)
...
def build_breakeven_state(breakevens: pd.DataFrame) -> pd.DataFrame:
    df = breakevens.set_index("date").sort_index()
    df = df[df.index < END_EXCLUSIVE].dropna(subset=["breakeven_inflation"]).copy()
    prior = df["breakeven_inflation"].shift(1)
    ...
    df["z"] = (df["breakeven_inflation"] - df["mean"]) / df["std"]
    risk_off = False
    states: list[bool] = []
    for z in df["z"]:
        if np.isfinite(z):
...
def write_report(results: list[dict], state: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        f"- **Real data:** FRED T10YIE cached at `{CACHE.relative_to(ROOT)}` ({state.index.min().date()} to {state.index.max().date()}); cached Binance spot daily OHLCV; no synthetic/proxy inputs.",
        "",
        "## Results",
        "",
...
def main() -> None:
    breakevens = fetch_or_load_t10yie()
    state = build_breakeven_state(breakevens)
    results = [evaluate_asset(asset, state) for asset in ASSETS]
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    out = pd.DataFrame(results)
    out.to_csv(RESULTS_DIR / "summary.csv", index=False)
    state.to_csv(RESULTS_DIR / "breakeven_inflation_state.csv")
