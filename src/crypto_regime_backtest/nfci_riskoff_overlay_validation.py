"""EXP-2026-09-06-NFCIRISKOFF-001: NFCI financial-conditions stress as crypto risk-off overlay.

Preregistered rule: use the real Chicago Fed National Financial Conditions Index
(FRED NFCI) as an external financial-stress de-risking overlay. If the NFCI
z-score is >= +2.0 versus a prior-only 156-week baseline, exit BTC/ETH/SOL/XRP
at the next daily open; stay cash until z < +1.0, then re-enter at the next daily
open. No synthetic or proxy inputs.
"""
...
CACHE = ROOT / "data" / "macro_financial_conditions" / "nfci_1w.csv.gz"
REPORT_PATH = ROOT / "docs" / "NFCI_RISKOFF_OVERLAY_VALIDATION.md"
RESULTS_DIR = ROOT / "results" / "nfci_riskoff_overlay"


def fetch_or_load_nfci() -> pd.DataFrame:
    CACHE.parent.mkdir(parents=True, exist_ok=True)
    if CACHE.exists():
        df = pd.read_csv(CACHE, parse_dates=["date"])
        df["date"] = pd.to_datetime(df["date"], utc=True)
        if df["date"].min() <= pd.Timestamp("1990-01-01T00:00:00Z") and df["date"].max() >= END_EXCLUSIVE - pd.Timedelta(days=120):
            return df
    payload = None
    ...
    raw.columns = ["date", "nfci"]
    raw = raw[raw["nfci"] != "."].copy()
    raw["date"] = pd.to_datetime(raw["date"], utc=True)
    raw["nfci"] = raw["nfci"].astype(float)
...
def build_nfci_state(nfci: pd.DataFrame) -> pd.DataFrame:
    df = nfci.set_index("date").sort_index()
    df = df[df.index < END_EXCLUSIVE].dropna(subset=["nfci"]).copy()
    prior = df["nfci"].shift(1)
    df["mean"] = prior.rolling(ROLLING_WEEKS, min_periods=ROLLING_WEEKS).mean()
    df["std"] = prior.rolling(ROLLING_WEEKS, min_periods=ROLLING_WEEKS).std(ddof=1)
    df["z"] = (df["nfci"] - df["mean"]) / df["std"]
    risk_off = False
    states: list[bool] = []
    for z in df["z"]:
        if np.isfinite(z):
    ...
    # Weekly FRED release is only actionable after the observation date; ffill to daily spot below.
...
def evaluate_asset(asset: str, state: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "double_cost": float(eq_double["equity"].iloc[-1]), "delay_2w": float(eq_delay["equity"].iloc[-1]),
...
def write_report(results: list[dict], state: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        f"- **Real data:** FRED NFCI cached at `{CACHE.relative_to(ROOT)}` ({state.index.min().date()} to {state.index.max().date()}); cached Binance spot daily OHLCV; no synthetic/proxy inputs.",
        "", "## Results", "",
    ...
    for r in results:
        lines.append(
            f"| {r['asset']} | {r['risk_off_days']} ({r['risk_off_fraction']:.1%}) | {r['trades']} | ${r['final']:,.2f} | ${r['buy_hold']:,.2f} | ${r['dca']:,.2f} | ${r['momentum']:,.2f} | ${r['random']:,.2f} | ${r['double_cost']:,.2f} | ${r['delay_2w']:,.2f} | ${r['test_final']:,.2f} vs ${r['test_buy_hold']:,.2f} ({r['test_trades']} trades) | {r['sharpe']:.2f} | {r['max_drawdown']:.1%} | {r['top_block_pct']:.1%} | Rejected |"
        )
    lines.extend([
        "", "## Decisive checks", "",
        ...
        f"- Concentration gate: {pass_conc}/4 assets cleared the 20% top-PnL-block cap.",
