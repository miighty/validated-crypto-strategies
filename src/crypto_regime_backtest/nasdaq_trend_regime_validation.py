"""EXP-2026-09-08-NASDAQTREND-001: Nasdaq Composite trend risk-on regime filter.

Preregistered rule: use real FRED NASDAQCOM daily closes only as an external growth-equity
risk appetite filter. Hold BTC/ETH/SOL/XRP long only when the Nasdaq Composite close is above
its prior-only 200-trading-day SMA; otherwise cash. The crypto action is delayed one
daily bar after the NASDAQCOM close signal. No synthetic/proxy data.
"""
...
BASE_SEED = 202609082
NASDAQ_URL = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=NASDAQCOM"
NASDAQ_CACHE = ROOT / "data" / "macro_nasdaq_trend" / "nasdaqcom_1d.csv.gz"
REPORT_PATH = ROOT / "docs" / "NASDAQ_TREND_REGIME_VALIDATION.md"
RESULTS_DIR = ROOT / "results" / "nasdaq_trend_regime"


def fetch_or_load_nasdaq() -> pd.DataFrame:
    NASDAQ_CACHE.parent.mkdir(parents=True, exist_ok=True)
    if NASDAQ_CACHE.exists():
        df = pd.read_csv(NASDAQ_CACHE, parse_dates=["date"])
        df["date"] = pd.to_datetime(df["date"], utc=True)
        df["nasdaq"] = pd.to_numeric(df["nasdaq"], errors="coerce")
        df = df.dropna(subset=["nasdaq"]).sort_values("date").drop_duplicates("date").reset_index(drop=True)
        df.to_csv(NASDAQ_CACHE, index=False, compression="gzip")
    ...
    for attempt in range(5):
        try:
            req = urllib.request.Request(NASDAQ_URL, headers={"User-Agent": "validated-crypto-strategies/0.1"})
    ...
    if payload is None:
        raise RuntimeError(f"Real FRED NASDAQCOM fetch failed: {last_error}")
    raw = pd.read_csv(StringIO(payload))
    raw.columns = ["date", "nasdaq"]
    raw["date"] = pd.to_datetime(raw["date"], utc=True)
    raw["nasdaq"] = pd.to_numeric(raw["nasdaq"], errors="coerce")
    raw = raw.dropna(subset=["nasdaq"])
    raw = raw.sort_values("date").drop_duplicates("date").reset_index(drop=True)
    raw.to_csv(NASDAQ_CACHE, index=False, compression="gzip")
    return raw
...
def build_nasdaq_state(nasdaq: pd.DataFrame) -> pd.DataFrame:
    df = nasdaq.set_index("date").sort_index()
    df = df[df.index < END_EXCLUSIVE].copy()
    prior = df["nasdaq"].shift(1)
    df["sma200_prior"] = prior.rolling(ROLLING_DAYS, min_periods=ROLLING_DAYS).mean()
    df["risk_on_signal_close"] = (df["nasdaq"] > df["sma200_prior"]).fillna(False).astype(bool)
...
def write_report(results: list[dict], state: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        f"- **Real data:** FRED NASDAQCOM cached at `{NASDAQ_CACHE.relative_to(ROOT)}` ({state.index.min().date()} to {state.index.max().date()}); cached Binance spot daily OHLCV; no synthetic/proxy inputs.",
        "",
        "## Results",
        "",
    ...
    lines.extend([
        ...
        "## Decisive checks",
        "",
        f"- Benchmark gate mixed: {beat_bh}/4 beat buy-and-hold and DCA, but only {beat_mom}/4 beat the BTC-momentum regime control.",
