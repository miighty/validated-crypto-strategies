"""EXP-2026-09-09-LQDTREND-001: investment-grade credit / LQD trend risk-on filter.

Preregistered rule: use real Yahoo Finance LQD adjusted daily closes as an external investment-grade credit
risk-appetite filter. Hold BTC/ETH/SOL/XRP long only when LQD adjusted close is above its
...
BASE_SEED = 2026090905
CACHE = ROOT / "data" / "macro_lqd_trend" / "lqd_1d.csv.gz"
REPORT_PATH = ROOT / "docs" / "LQD_TREND_REGIME_VALIDATION.md"
RESULTS_DIR = ROOT / "results" / "lqd_trend_regime"


def fetch_or_load_lqd() -> pd.DataFrame:
    CACHE.parent.mkdir(parents=True, exist_ok=True)
    if CACHE.exists():
        df = pd.read_csv(CACHE, parse_dates=["date"])
        df["date"] = pd.to_datetime(df["date"], utc=True)
        df["lqd"] = pd.to_numeric(df["lqd"], errors="coerce")
        df = df.dropna(subset=["lqd"]).sort_values("date").drop_duplicates("date").reset_index(drop=True)
    ...
    url = f"https://query1.finance.yahoo.com/v8/finance/chart/LQD?period1={period1}&period2={period2}&interval=1d&events=history&includeAdjustedClose=true"
    ...
    if payload is None:
        raise RuntimeError(f"Real Yahoo Finance LQD fetch failed: {last_error}")
    ...
    raw = pd.DataFrame({"date": pd.to_datetime(chart["timestamp"], unit="s", utc=True).normalize(), "lqd": adj or close})
    raw["lqd"] = pd.to_numeric(raw["lqd"], errors="coerce")
    raw = raw.dropna(subset=["lqd"]).sort_values("date").drop_duplicates("date").reset_index(drop=True)
...
def build_state(lqd: pd.DataFrame) -> pd.DataFrame:
    df = lqd.set_index("date").sort_index()
    df = df[df.index < END_EXCLUSIVE].copy()
    prior = df["lqd"].shift(1)
    df["sma200_prior"] = prior.rolling(ROLLING_DAYS, min_periods=ROLLING_DAYS).mean()
    df["risk_on_signal_close"] = (df["lqd"] > df["sma200_prior"]).fillna(False).astype(bool)
...
def write_report(results: list[dict], state: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        f"- **Real data:** Yahoo Finance LQD adjusted close cached at `{CACHE.relative_to(ROOT)}` ({state.index.min().date()} to {state.index.max().date()}); cached Binance spot daily OHLCV; no synthetic/proxy inputs.",
        "", "## Results", "",
