"""EXP-2026-09-07-VXNRISKOFF-001: Nasdaq-100 implied-volatility stress risk-off overlay.

Preregistered rule: use real FRED/CBOE VXNCLS only as an external tech-equity
stress de-risking overlay. If VXN z-score >= +2.0 versus a prior-only
60-trading-day baseline, exit crypto at the next daily open and stay in cash until
VXN z-score falls below +1.0; then re-enter at the next daily open. No synthetic
or proxy data.
"""
...
BASE_SEED = 202609072
VXN_URL = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=VXNCLS"
VXN_CACHE = ROOT / "data" / "macro_vxn" / "vxncls_1d.csv.gz"
REPORT_PATH = ROOT / "docs" / "VXN_RISKOFF_OVERLAY_VALIDATION.md"
RESULTS_DIR = ROOT / "results" / "vxn_riskoff_overlay"


def fetch_or_load_vxn() -> pd.DataFrame:
    VXN_CACHE.parent.mkdir(parents=True, exist_ok=True)
    if VXN_CACHE.exists():
        df = pd.read_csv(VXN_CACHE, parse_dates=["date"])
        df["date"] = pd.to_datetime(df["date"], utc=True)
        df["vxn"] = pd.to_numeric(df["vxn"], errors="coerce")
        df = df.dropna(subset=["vxn"]).sort_values("date").drop_duplicates("date").reset_index(drop=True)
        df.to_csv(VXN_CACHE, index=False, compression="gzip")
    ...
    for attempt in range(5):
        try:
            req = urllib.request.Request(VXN_URL, headers={"User-Agent": "validated-crypto-strategies/0.1"})
    ...
    if payload is None:
        raise RuntimeError(f"Real FRED VXNCLS fetch failed: {last_error}")
    raw = pd.read_csv(StringIO(payload))
    raw.columns = ["date", "vxn"]
    raw["date"] = pd.to_datetime(raw["date"], utc=True)
    raw["vxn"] = pd.to_numeric(raw["vxn"], errors="coerce")
    raw = raw.dropna(subset=["vxn"])
    raw = raw.sort_values("date").drop_duplicates("date").reset_index(drop=True)
    raw.to_csv(VXN_CACHE, index=False, compression="gzip")
    return raw
...
def build_vxn_state(vxn: pd.DataFrame) -> pd.DataFrame:
    df = vxn.set_index("date").sort_index()
    df = df[df.index < END_EXCLUSIVE].copy()
    prior = df["vxn"].shift(1)
...
def align_risk_off(spot_index: pd.DatetimeIndex, state: pd.DataFrame) -> pd.Series:
    return state["risk_off_actionable"].reindex(spot_index, method="ffill").fillna(False).astype(bool)
...
def write_report(results: list[dict], state: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        f"- **Real data:** FRED VXNCLS cached at `{VXN_CACHE.relative_to(ROOT)}` ({state.index.min().date()} to {state.index.max().date()}); cached Binance spot daily OHLCV; no synthetic/proxy inputs.",
        "",
        "## Results",
        "",
