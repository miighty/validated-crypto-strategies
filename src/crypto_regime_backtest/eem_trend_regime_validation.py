"""EXP-2026-09-09-EEMTREND-001: emerging-market equities / EEM trend risk-on filter.

Preregistered rule: use real Yahoo Finance EEM adjusted daily closes as an external global-risk
risk-appetite filter. Hold BTC/ETH/SOL/XRP long only when EEM adjusted close is above its
...
BASE_SEED = 2026090903
CACHE = ROOT / "data" / "macro_eem_trend" / "eem_1d.csv.gz"
REPORT_PATH = ROOT / "docs" / "EEM_TREND_REGIME_VALIDATION.md"
RESULTS_DIR = ROOT / "results" / "eem_trend_regime"


def fetch_or_load_eem() -> pd.DataFrame:
    CACHE.parent.mkdir(parents=True, exist_ok=True)
    if CACHE.exists():
        df = pd.read_csv(CACHE, parse_dates=["date"])
        df["date"] = pd.to_datetime(df["date"], utc=True)
        df["eem"] = pd.to_numeric(df["eem"], errors="coerce")
        df = df.dropna(subset=["eem"]).sort_values("date").drop_duplicates("date").reset_index(drop=True)
        if df["date"].max() >= END_EXCLUSIVE - pd.Timedelta(days=30):
            return df
    last_error: Exception | None = None
    payload = None
    period1 = int(pd.Timestamp("2003-04-11", tz="UTC").timestamp())
    period2 = int(END_EXCLUSIVE.timestamp())
    url = f"https://query1.finance.yahoo.com/v8/finance/chart/EEM?period1={period1}&period2={period2}&interval=1d&events=history&includeAdjustedClose=true"
    ...
    if payload is None:
        raise RuntimeError(f"Real Yahoo Finance EEM fetch failed: {last_error}")
    ...
    raw = pd.DataFrame({"date": pd.to_datetime(chart["timestamp"], unit="s", utc=True).normalize(), "eem": adj or close})
    raw["eem"] = pd.to_numeric(raw["eem"], errors="coerce")
    raw = raw.dropna(subset=["eem"]).sort_values("date").drop_duplicates("date").reset_index(drop=True)
...
def build_state(eem: pd.DataFrame) -> pd.DataFrame:
    df = eem.set_index("date").sort_index()
    df = df[df.index < END_EXCLUSIVE].copy()
    prior = df["eem"].shift(1)
    df["sma200_prior"] = prior.rolling(ROLLING_DAYS, min_periods=ROLLING_DAYS).mean()
    df["risk_on_signal_close"] = (df["eem"] > df["sma200_prior"]).fillna(False).astype(bool)
...
def write_report(results: list[dict], state: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        f"- **Real data:** Yahoo Finance EEM adjusted close cached at `{CACHE.relative_to(ROOT)}` ({state.index.min().date()} to {state.index.max().date()}); cached Binance spot daily OHLCV; no synthetic/proxy inputs.",
        "", "## Results", "",
