"""EXP-2026-09-07-STLFSIRISKOFF-001: St. Louis Fed FSI stress as crypto risk-off overlay.

Preregistered rule: use real FRED St. Louis Fed Financial Stress Index
(STLFSI4) as an external broad-market financial-stress de-risking overlay. If
the index z-score is >= +2.0 versus a prior-only 156-week baseline, exit
BTC/ETH/SOL/XRP at the next daily open; stay cash until z < +1.0, then
re-enter at the next daily open. No synthetic/proxy inputs.
"""
...
BASE_SEED = 202609074
FRED_SERIES = "STLFSI4"
FRED_URL = f"https://fred.stlouisfed.org/graph/fredgraph.csv?id={FRED_SERIES}&cosd=2010-01-01"
CACHE = ROOT / "data" / "macro_stress" / "stlouis_fsi_1w.csv.gz"
REPORT_PATH = ROOT / "docs" / "STLOUIS_FSI_RISKOFF_VALIDATION.md"
RESULTS_DIR = ROOT / "results" / "stlouis_fsi_riskoff"
...
def fetch_or_load_credit_spread() -> pd.DataFrame:
    ...
    if CACHE.exists():
        ...
        if df["date"].min() <= pd.Timestamp("2010-01-01T00:00:00Z") and df["date"].max() >= END_EXCLUSIVE - pd.Timedelta(days=90):
            return df
    payload = None
...
def write_report(results: list[dict], state: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        f"- **Real data:** FRED STLFSI4 cached at `{CACHE.relative_to(ROOT)}` ({state.index.min().date()} to {state.index.max().date()} after the 156-week prior-only warmup; raw FRED graph export provides 2010 onward); cached Binance spot daily OHLCV; no synthetic/proxy inputs.",
        "", "## Results", "",
