...
class StudyConfig:
    ...
    primary_rule: str = (
        ...
        " have funding >= +5 bps, select the single most positive asset, enter spot long at the next hourly"
        " open with the full accrued reserve, hold 8h, then exit at the close and remain in cash for a 24h"
        " cooldown."
    )


def run_funding_positive_panel_validation(paths: Paths, seed: int = RANDOM_SEED) -> pd.DataFrame:
    paths.create()
    run_id = pd.Timestamp.now(tz="UTC").strftime("run-%Y%m%dT%H%M%SZ")
    output = paths.results / "funding_positive_panel" / "runs" / run_id
    ...
    print(f"Funding-positive panel validation written to {output}", flush=True)
...
def run_hostile_checks(
    ...
    doubled_cost = simulate_signal_strategy(
        ...
        final_closes,
        signals,
        "positive_funding_persistence_panel_doubled_cost",
    )
    without_best = remove_best_trade(market, schedule, one_way_cost, final_closes, signals)
    random_baseline = simulate_random_baseline(
        market, schedule, one_way_cost, final_closes, signals, seed
    )
    return pd.DataFrame(
        [
            {
...
def write_report(
    output: Path, manifest: dict[str, object], summary: pd.DataFrame, hostile: pd.DataFrame
) -> None:
    primary = summary.loc[summary["strategy"].eq("positive_funding_persistence_panel")].iloc[0]
    ...
    lines = [
        ...
        "## Honest conclusion",
        "",
        f"> **{primary['verdict']}.** The positive-funding persistence rule finished at **${primary['terminal_value']:.2f}** across **{int(primary['event_count'])} trades**. The decisive benchmark gate remains the best required DCA baseline at **${baselines['terminal_value'].max():.2f}**.",
        "",
