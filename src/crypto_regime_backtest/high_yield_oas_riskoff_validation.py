"""EXP-2026-09-07-HYOASRISKOFF-001: high-yield OAS stress as crypto risk-off overlay.

Preregistered rule: use real FRED ICE BofA US High Yield Index Option-Adjusted
Spread (BAMLH0A0HYM2) as an external below-investment-grade credit-stress
de-risking overlay. If the spread z-score is >= +2.0 versus a prior-only
252-trading-day baseline, exit BTC/ETH/SOL/XRP at the next daily open; stay cash
until z < +1.0, then re-enter at the next daily open. No synthetic/proxy inputs.
"""
...
FRED_URL = f"https://fred.stlouisfed.org/graph/fredgraph.csv?id={FRED_SERIES}&cosd=1996-12-31"
CACHE = ROOT / "data" / "macro_credit" / "high_yield_oas_1d.csv.gz"
REPORT_PATH = ROOT / "docs" / "HIGH_YIELD_OAS_RISKOFF_VALIDATION.md"
RESULTS_DIR = ROOT / "results" / "high_yield_oas_riskoff"
...
def fetch_or_load_credit_spread() -> pd.DataFrame:
    ...
    if CACHE.exists():
        ...
        if df["date"].min() <= pd.Timestamp("2023-09-05T00:00:00Z") and df["date"].max() >= END_EXCLUSIVE - pd.Timedelta(days=90):
            return df
    payload = None
    ...
    raw.columns = ["date", "credit_spread"]
    raw = raw[raw["credit_spread"] != "."].copy()
    raw["date"] = pd.to_datetime(raw["date"], utc=True)
    raw["credit_spread"] = raw["credit_spread"].astype(float)
...
def write_report(results: list[dict], state: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        f"- **Real data:** FRED BAMLH0A0HYM2 cached at `{CACHE.relative_to(ROOT)}` ({state.index.min().date()} to {state.index.max().date()} after the 252-day prior-only warmup; raw FRED graph export currently provides 2023-09-05 onward); cached Binance spot daily OHLCV; no synthetic/proxy inputs.",
        "", "## Results", "",
