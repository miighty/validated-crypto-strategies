"""EXP-2026-09-07-OILVOLRISKOFF-001: oil-implied-volatility risk-off overlay.

Preregistered rule: use real FRED CBOE Crude Oil ETF Volatility Index (OVXCLS) as
an external commodity/energy stress signal. If OVX z-score is >= +2.0 versus a
...
CACHE = ROOT / "data" / "macro_oil_vol" / "ovxcls_1d.csv.gz"
REPORT_PATH = ROOT / "docs" / "OIL_VOL_RISKOFF_VALIDATION.md"
RESULTS_DIR = ROOT / "results" / "oil_vol_riskoff"


def fetch_or_load_ovx() -> pd.DataFrame:
    CACHE.parent.mkdir(parents=True, exist_ok=True)
    if CACHE.exists():
        df = pd.read_csv(CACHE, parse_dates=["date"])
        df["date"] = pd.to_datetime(df["date"], utc=True)
        if df["date"].min() <= pd.Timestamp("2007-05-10T00:00:00Z") and df["date"].max() >= END_EXCLUSIVE - pd.Timedelta(days=14):
            return df
    payload = None
    ...
    raw = raw[raw["oil_vol"] != "."].copy()
    raw["date"] = pd.to_datetime(raw["date"], utc=True)
    raw["oil_vol"] = pd.to_numeric(raw["oil_vol"], errors="coerce")
    raw = raw.dropna(subset=["oil_vol"]).sort_values("date").drop_duplicates("date").reset_index(drop=True)
...
def build_ovx_state(ovx: pd.DataFrame) -> pd.DataFrame:
    df = ovx.set_index("date").sort_index()
    df = df[df.index < END_EXCLUSIVE].dropna(subset=["oil_vol"]).copy()
    prior = df["oil_vol"].shift(1)
    ...
    df["z"] = (df["oil_vol"] - df["mean"]) / df["std"]
    risk_off = False
    states: list[bool] = []
    for z in df["z"]:
        if np.isfinite(z):
...
def write_report(results: list[dict], state: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        f"- **Real data:** FRED OVXCLS cached at `{CACHE.relative_to(ROOT)}` ({state.index.min().date()} to {state.index.max().date()}); cached Binance spot daily OHLCV; no synthetic/proxy inputs.",
        "",
        "## Results",
        "",
