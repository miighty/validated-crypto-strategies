...
PARTITIONS = (Partition("full_sample_2023_2024", SAMPLE_START, None),)


def run_btc_etf_validation(paths: Paths) -> pd.DataFrame:
    paths.create()
    run_id = pd.Timestamp.now(tz="UTC").strftime("run-%Y%m%dT%H%M%SZ")
    output = paths.results / "btc_etf" / "runs" / run_id
    output.mkdir(parents=True, exist_ok=False)

    config = StudyConfig()
    metadata = fetch_market_metadata(paths)
    hourly = load_or_fetch_hourly_series(paths, metadata)
    etf = hourly[hourly["family"].eq("btc_etf")].copy().sort_values("timestamp")
    etf = etf[etf["timestamp"] >= pd.Timestamp(config.sample_start)].copy()
    if etf.empty:
        raise RuntimeError("No Bitcoin ETF Polymarket hourly series found")

    market_end = pd.Timestamp(etf["timestamp"].max())
    study_end = market_end + pd.Timedelta(hours=max(SENSITIVITY_HOLDS))
    ...
    primary = simulate_btc_etf_strategy(btc_reset, etf, schedule, config.one_way_cost, primary_spec)
    ...
    pd.concat(
        [
            ...
            primary.ledger.assign(strategy="btc_etf_odds_btc_swing"),
        ],
        ignore_index=True,
    ...
    pd.concat(
        [
            ...
            primary.equity.assign(strategy="btc_etf_odds_btc_swing"),
        ],
        ignore_index=True,
    ...
    pd.concat(
        [
            ...
            primary.partition_summary.assign(strategy="btc_etf_odds_btc_swing"),
        ],
        ignore_index=True,
    ...
    etf[["timestamp", "slug", "yes_price", "trade_count", "traded_notional"]].to_csv(
        output / "btc_etf_hourly_odds.csv", index=False, float_format="%.17g"
    )
    ...
    manifest = {
        ...
        "market_slug": str(etf["slug"].iloc[0]),
        "market_question": str(etf["question"].iloc[0]),
    }
    ...
    print(f"BTC ETF-odds validation written to {output}", flush=True)
    return summaries
...
def build_btc_etf_signals(hourly: pd.DataFrame, spec: StrategySpec) -> pd.DataFrame:
...
def simulate_btc_etf_strategy(
    ...
    signals = build_btc_etf_signals(hourly, spec)
        ...
        for delta in SENSITIVITY_DELTAS:
            for level in SENSITIVITY_LEVELS:
                for hold in SENSITIVITY_HOLDS:
                    spec = StrategySpec(
                        name=f"btc_etf_l{lookback}_d{int(delta*1000)}_lv{int(level*100)}_h{hold}",
                        lookback_hours=lookback,
                        delta_threshold=delta,
                        level_threshold=level,
                        hold_hours=hold,
                    )
                    result = simulate_btc_etf_strategy(btc, hourly, schedule, one_way_cost, spec)
                    ...
                    rows.append(summary)
    doubled = simulate_btc_etf_strategy(
        btc,
        hourly,
        schedule,
        one_way_cost * 2,
        StrategySpec(
            "btc_etf_doubled_cost",
...
def classify_verdict(
    ...
    if primary.summary["event_count"] == 0:
        label = "rejected"
        reason = "Primary Bitcoin ETF swing rule never triggered in the sample."
    elif primary.summary["event_count"] < 3:
        label = "inconclusive"
        reason = "Positive signal exists, but fewer than 3 trades is not enough to validate against BTC DCA."
    elif primary_beats_both and nearby_pass_rate >= 0.5 and hostile_pass and doubled_pass:
        label = "paper-trading candidate"
        reason = "Primary ETF-odds swing rule beat both BTC DCA baselines and cleared the main robustness gates."
    else:
        label = "rejected"
...
def write_report(
    ...
    primary = summaries.loc[summaries["strategy"] == "btc_etf_odds_btc_swing"].iloc[0]
    lines = [
        ...
        f"| BTC ETF odds → BTC swing | {primary['final_usd_value']:.2f} | {primary['final_btc_equivalent']:.8f} | {primary['total_costs_usd']:.2f} | {int(primary['event_count'])} | {primary['max_drawdown']:.2%} | {primary['final_btc_equivalent'] - daily['final_btc_equivalent']:.8f} | {primary['final_btc_equivalent'] - weekly['final_btc_equivalent']:.8f} |",
        "",
        "## Verdict",
        "",
